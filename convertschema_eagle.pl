...
our $timestamp =
  $start_time; # this value gets decreased every time we need a unique timestamp
my %hvmap = ( "0" => "H", "1" => "V", "2" => "H", "3" => "V" );
our %uniquereferences = ();
my %myrot = ( "0" => "0", "90" => "1", "270" => "2" );
my %iotypes = ( "0" => "BiDi", "1" => "Output", "2" => "Input", "3" => "BiDi" )
  ;            # Others unknown yet (0 is really 'unspecified' in Altium)
my %partparams = ();
...
sub get_f_position(\%$$$$$) {
    ...
    my $x = (
        (
            ( $d{'LOCATION.X'} || 0 ) +
              ( ( $d{'LOCATION.X_FRAC'} || 0 ) / 100000.0 )
        ) * $f
    );
    my $y = (
        (
            ( $d{'LOCATION.Y'} || 0 ) +
              ( ( $d{'LOCATION.Y_FRAC'} || 0 ) / 100000.0 )
        ) * $f
    );
 ...
 #print "LOC.Y: ".($d{'LOCATION.Y'}*$f)." rely: $rely sheety=$sheety\n";
    ...
    $ownrot2 = ( $d{'ORIENTATION'} ) % 4
      if ( $d{'ORIENTATION'} )
      ;    # This line is most likely buggy and should be improved
           #ext if($ownrot!=$ownrot2);
           #next unless($d{'ORIENTATION'});
           #next if($ownrot!=1);

#print $d{'TEXT'}." -> $t -> $nameLabels{$LIBREFERENCE}\n"; # if($d{'NAME'} eq "Rule");
    ...
    glob('"*.schdoc"') )
{
    ...
    next
      if ( $content =~ m/EESchema Schematic File Version/ )
      ;                                      # Skipping KiCad schematics
    next
      if ( $content =~ m/PCB \d+.\d+ Binary Library File/ )
      ;                                      # Skipping PCB Files
    next if ( substr( $content, 0, 2 ) eq "\xD0\xCF" );
    next
      if ( ( !$protel )
        && unpack( "l", substr( $content, 0, 4 ) ) > length($content) );
...
EOF
    ...
    my %formats = (
        ...
        8  => "D 34000 22000",
        9  => "E 44000 34000",
        10 => "USLetter 11000 8500",
        11 => "USLegal 14000 8500",
        12 => "USLedger 17000 11000",
        13 => "A 11000 8500",
        14 => "B 17000 11000",
        15 => "C 22000 17000",
        16 => "D 34000 22000",
    ...
    my $sheetstyle = 6;
    $sheetstyle = $1 if ( $text =~ m/SHEETSTYLE=(\d+)/ );
    ...
    if ( $text =~ m/WORKSPACEORIENTATION=1/ ) {
        $sheetformat = "$1 $3 $2 portrait"
          if ( $sheetformat =~ m/(\w+) (\d+) (\d+)/ );
    }

    my $sheety = 12000;
    $sheety = $1 if ( $sheetformat =~ m/\w+ \d+ (\d+)/ );
    ...
    sub mapDir($$$) {
        ...
        my $debug = $_[2];
        my %dirmap =
          ( "0" => "L BNN", "1" => "R TNN", "2" => "R TNN", "3" => "L BNN" );
        my %dirmapmirrored =
          ( "0" => "R BNN", "1" => "L TNN", "2" => "L TNN", "3" => "R BNN" );
...
#print "Mapping $_[0] (Mirror: $mirrored) to: ".($mirrored? $dirmapmirrored{$_[0]}:$dirmap{$_[0]})."\n" if($debug);
    ...
    foreach my $b (@a) {
        ...
        print LOG sprintf(
            "RECORD=%2d|LINENO=%4d|OWNERPARTID=%4d|OWNERINDEX=%4d|%s\n",
            defined( $d{'RECORD'} ) ? $d{'RECORD'} : -42,
            $d{'LINENO'},
            defined( $d{'OWNERPARTID'} ) ? $d{'OWNERPARTID'} + 1 : -42,
            defined( $d{'OWNERINDEX'} )  ? $d{'OWNERINDEX'} + 1  : -42,
            $o
        ) if ($USELOGGING);
        ...
        sub drawcomponent($) {
            $componentdraw{ $LIBREFERENCE || 0 } .= $_[0]
              unless (
                defined( $componentcontains{ $LIBREFERENCE || 0 }{ $_[0] } ) );
...
#print "Checking for\nOWNERINDEX: $d{'OWNERINDEX'} vs. ".($OWNERLINENO-1)." ?\n $OWNERPARTDISPLAYMODE vs. ".($d{'OWNERPARTDISPLAYMODE'}||-1)." ?\n";
            next
              if ( ( ( $d{'OWNERINDEX'} || 0 ) eq $OWNERLINENO - 1 )
                && ( $d{'OWNERPARTDISPLAYMODE'} || -1 ) ne
                $OWNERPARTDISPLAYMODE );
        }

        if ( ( $d{'OWNERPARTID'} || "" ) eq "0" ) {
            print "Warning: Edge case: OWNERPARTID="
              . ( defined( $d{'OWNERPARTID'} ) ? $d{'OWNERPARTID'} : "" )
              . " RECORD=$d{'RECORD'} the behaviour has been changed on 2018-01-05. If this causes a regression, please file an issue.\n";
        }
...
#|RECORD=4|LOCATION.X=40|TEXT=I2C mappings:|OWNERPARTID=-1|INDEXINSHEET=26|COLOR=8388608|LOCATION.Y=500|FONTID=3
                ...
                drawcomponent
                  "T $o $x $y $size 0 1 1 \"$value\" Normal 0 L B\n";
            }
...
#|RECORD=32|LOCATION.X=40|TEXT=U_02cpu_power|OWNERINDEX=42|OWNERPARTID=-1|COLOR=8388608|INDEXINSHEET=-1|LOCATION.Y=240|FONTID=1
            ...
            elsif ( $d{'RECORD'} eq '13' )           # Line
            {
                ...
                my $x = mil2mm(( $d{'LOCATION.X'} * $f ) - $relx);
                my $y = mil2mm(( $d{'LOCATION.Y'} * $f ) - $rely);
                ( $x, $y ) = rotate( $x, $y, $partorientation{$globalp} );
                my $cx = mil2mm(( $d{'CORNER.X'} * $f ) - $relx) + $x;
                my $cy = mil2mm(( $d{'CORNER.Y'} * $f ) - $rely) + $y;
                ( $cx, $cy ) = rotate( $cx, $cy, $partorientation{$globalp} );
                drawcomponent "<wire x1=\"$x\" y1=\"$y\" x2=\"$cx\" y2=\"$cy\" width=\"0.127\" layer=\"94\"/>";
            }
            elsif ( $d{'RECORD'} eq '14' )           # Rectangle
            {
                ...
                my $x2 = mil2mm( $x - $cx );
                my $y2 = mil2mm( $y - $cy );
                drawcomponent "<wire x1=\"$x\" y1=\"$y\" x2=\"$x2\" y2=\"$y\" width=\"0.254\" layer=\"94\"/>\n";
                drawcomponent "<wire x1=\"$x2\" y1=\"$y\" x2=\"$x2\" y2=\"$y2\" width=\"0.254\" layer=\"94\"/>\n";
                drawcomponent "<wire x1=\"$x2\" y1=\"$y2\" x2=\"$x\" y2=\"$y2\" width=\"0.254\" layer=\"94\"/>\n";
                drawcomponent "<wire x1=\"$x\" y1=\"$y2\" x2=\"$x\" y2=\"$y\" width=\"0.254\" layer=\"94\"/>\n";
            }
            ...
            elsif ( $d{'RECORD'} eq '2' )    # Pin
            {
                ...
                while ( $namepos < length($oldname) && $namepos >= 0 ) {
                    my $thisstate =
                      ( defined( substr( $oldname, $namepos + 1, 1 ) )
                          && substr( $oldname, $namepos + 1, 1 ) eq "\\" )
                      ? 1
                      : 0;
                    if ( $thisstate != $state ) {
                        $name .= "~";
                    ...
                    $state = $thisstate;
                    $name .= substr( $oldname, $namepos, 1 )
                      unless ( substr( $oldname, $namepos, 1 ) eq "\\"
                        && !$thisstate );
...
#print "oldname: $oldname brokenname:$brokenname name:$name\n" if($brokenname ne $name);
                    ...
                    && defined( $d{'LOCATION.Y'} ) )
                {
                    ...
                    my %map2 =
                      ( "0" => "0", "1" => "3", "2" => "2", "3" => "1" );
                    $pinorient +=
                      $map2{ ( $partorientation{$globalp} || 0 ) & 3 };
                    $pinorient &= 3;
                    my $mirrored   = ( $partorientation{$globalp} || 0 ) & 4;
                    my $dir        = $dirtext{$pinorient};
                    my $x          = mil2mm($d{'LOCATION.X'} * $f);
                    my $y          = mil2mm($d{'LOCATION.Y'} * $f);
                    my $pinlength  = mil2mm(( $d{'PINLENGTH'} || 1 ) * $f);
                    my $electrical = "U";
                    my $rotation = "R0";
                    ...
                    if ($pinlength < 0.01) {
                        $pinlength = "point";
                    } elsif ($pinlength < 2.54) {
                        $pinlength = "short";
                    } elsif ($pinlength < 5.12) {
                        $pinlength = "middle";
                    } else {
                        # > 5.12 mm 
                        $pinlength = "long";
                    ...
                    $rotation = $rotationMap{ $dir };
                    $E = $electricmap{ $d{'ELECTRICAL'} } || "I"
                    if ( defined( $d{'ELECTRICAL'} ) );
                    my $F = ""
                      ; # $F=" F" if($d{'ELECTRICAL'}eq "7"); Unfortunately, Altium and KiCad have different meanings for the same symbols
                    ...
                    drawcomponent "<pin name=\"$name\" x=\"$x\" y=\"$y\" visible=\"pin\" length=\"$pinlength\" rot=\"$rotation\"/>\n";
                }
                else {
            ...
            elsif ( $d{'RECORD'} eq '3' )    # Pin symbol
            {
                ...
                || $d{'RECORD'} eq '5' )    # Polyline or Bezier!
            {
                ...
                if ( $d{'RECORD'} eq '5' );
                my $fill =
                  ( defined( $d{'ISSOLID'} ) && $d{'ISSOLID'} eq 'T' )
                  ? "F"
                  : "N";
                my $cmpd = "P "
                  . ( $d{'LOCATIONCOUNT'} || 0 ) . " 0 1 "
                  . ( $d{'LINEWIDTH'} || 1 ) . "0 ";
            ...
            elsif ( $d{'RECORD'} eq '7' )    #Polygon
            {
                ...
                my $lwidth =
                  defined( $d{'LINEWIDTH'} ) ? $d{'LINEWIDTH'} * 10 : 10;
            ...
            elsif ($d{'RECORD'} eq '12'
                || $d{'RECORD'} eq '11'
              ) # Arc or Elliptical arc (we average the axes as KiCad doesn't support it)
            {
                ...
                my $r = int(
                    (
                        ( $d{'RADIUS'} || 0 ) +
                          ( ( $d{'RADIUS_FRAC'} || 0 ) / 100000.0 )
                    ) * $f
                );
                my $sa = "0";
                $sa = "$1$2"
                  if ( defined( $d{'STARTANGLE'} )
                    && $d{'STARTANGLE'} =~ m/(\d+)\.(\d)(\d+)/ );
                my $ea = "3600";
                $ea = "$1$2"
                  if ( defined( $d{'ENDANGLE'} )
                    && $d{'ENDANGLE'} =~ m/(\d+)\.(\d)(\d+)/ );
                ...
                if ( $d{'RECORD'} eq '11' ) {
                    my $sc = int(
                        (
                            ( $d{'SECONDARYRADIUS'} || 0 ) + (
                                ( $d{'SECONDARYRADIUS_FRAC'} || 0 ) / 100000.0
                            )
                        ) * $f
                    );
                    $r = ( $r + $sc ) / 2;
                ...
                foreach (@liste) {
                    ...
                    $sa =
                      3600 -
                      $winkel{ ( $partorientation{$globalp} || 0 ) & 3 } * 10 +
                      $sa;
                    $sa %= 3600;
                    $sa -= 3600 if ( $sa > 1800 );
                    $ea =
                    ...
                    my $fill =
                      ...
                      : "N";
                    drawcomponent
"A $x $y $r $sa $ea 1 1 $d{LINEWIDTH}0 $fill $x1 $y1 $x2 $y2\n";
                }
            }
            elsif ( $d{'RECORD'} eq '41' )    # Text / Designator?
            {
                ...
                ( $x, $y ) = rotate( $x, $y, $partorientation{$globalp} );
                my $text = $d{'DESCRIPTION'} || $d{'TEXT'} || "";
                $text =~ s/\~/~~/g;
                $text =~ s/\~1/\~/g;
                $text =~ s/ /\~/g;
                if ( $d{'NOTAUTOPOSITION'} ) {
                    my $rot = $d{'ORIENTATION'}
                      || $myrot{ $fontrotation{ $d{'FONTID'} } };
                    my $size = $fontsize{ $d{'FONTID'} } * 6;
                    my $bold = $fontbold{ $d{'FONTID'} } ? "12" : "0";
                    print OUT "Text Notes "
                      . ( $d{'LOCATION.X'} * $f ) . " "
                      . ( $sheety - $d{'LOCATION.Y'} * $f )
                      . " $rot    $size   ~ $bold\n$text\n"
                      if ( $text ne "" && $text ne " " );
...
#LOCATION.X=1380|LOCATION.Y=520|PARTCOUNT=2|PARTIDLOCKED=F|SHEETPARTFILENAME=*|SOURCELIBRARYNAME=*|TARGETFILENAME=*|
                $LIBREFERENCE = $d{'LIBREFERENCE'};
                $LIBREFERENCE =~ s/ /_/g;
                $LIBREFERENCE .= "_" . $d{'CURRENTPARTID'}
                  if ( $d{'PARTCOUNT'} > 2 );
                $CURRENTPARTID        = $d{'CURRENTPARTID'} || undef;
                $OWNERPARTDISPLAYMODE = $d{'DISPLAYMODE'};
                $OWNERLINENO          = $d{'LINENO'};
                $globalp++;
                $nextxypos = ( $d{'LOCATION.X'} * $f ) . " "
                  . ( $sheety - $d{'LOCATION.Y'} * $f );
                $partorientation{$globalp} = $d{'ORIENTATION'} || 0;
                $partorientation{$globalp} += 4
                  if ( defined( $d{'ISMIRRORED'} ) && $d{'ISMIRRORED'} eq 'T' );
...
#|RECORD=4|LOCATION.X=40|TEXT=I2C mappings:|OWNERPARTID=-1|INDEXINSHEET=26|COLOR=8388608|LOCATION.Y=500|FONTID=3
                ...
                my $bold = $fontbold{ $d{'FONTID'} } ? "12" : "0";
                my $rot =
                  $d{'ORIENTATION'} || $myrot{ $fontrotation{ $d{'FONTID'} } };
...
#$dat="Text Label ".($d{'LOCATION.X'}*$f)." ".($sheety-$d{'LOCATION.Y'}*$f)." 0    60   ~ 0\n".$d{'TEXT'}."\n";
            ...
            {
                $prevfilename = $d{'TEXT'} if ( $d{'RECORD'} eq '33' );
                $prevfilename =~ s/\.SchDoc/-SchDoc\.sch/i;
                $dat =
"$symbol\nF0 \"$prevname\" 60\nF1 \"$prevfilename\" 60\n\$EndSheet\n";
...
EOF
                          ;
                        print LIB $comp
                          unless ( defined( $localcontains{$component} ) );
                        $localcontains{$component} = 1;
                        $globalcomp .= $comp
                          unless ( defined( $globalcontains{$component} ) );
                    ...
                    $refdesLabels{$device}  = "\"#PWR\" 0 140 20 H I L BNN";
                    $nameLabels{$device}    = "\"$device\" 0 110 30 H V L BNN";
...
EOF
                ...
                elsif ( $d{'TEXT'} eq 'VCOREDIG') 
                {
                    # This is a workaround for Novena, it could potentially break other schematics
                    $voltage = "1.5V";
                    $device  = "+1.5V";
                }
                else {
...
EOF
            ...
            elsif ( $d{'RECORD'} eq '8' )    # Ellipse
            {
                ...
                for ( my $i = 0 ; $i < $parts ; $i++ ) {
                    my $x1 =
                      int( $x + sin( ( 2 * $pi * $i / $parts ) ) * $radius );
                    my $y1 =
                      int( $y + cos( ( 2 * $pi * $i / $parts ) ) * $secondary );
                    my $x2 =
                      int( $x +
                          sin( ( 2 * $pi * ( $i + 1 ) / $parts ) ) * $radius );
                    my $y2 =
                      int( $y +
                          cos( ( 2 * $pi * ( $i + 1 ) / $parts ) ) *
                          $secondary );
            ...
            elsif ( $d{'RECORD'} eq '11' )    # Elliptic Arc
            {
                ...
                my $radius = int( $d{'RADIUS'} ) * $f;
                my $secondary =
                  int( $d{'SECONDARYRADIUS'} ) *
                  $f
                  ; # Primary and Secondary might be mixed up in the calculation
                my $sa = ( $d{'STARTANGLE'} + 90 ) * $pi / 180;
                my $ea = ( $d{'ENDANGLE'} + 90 ) * $pi / 180;
                $ea += $pi * 2 if ( $ea < $sa );
                my $ra = $ea - $sa;
                ...
                for ( my $i = 0 ; $i < $parts ; $i++ ) {
                    my $x1 =
                      int( $x + sin( $sa + ( $ra * $i / $parts ) ) * $radius );
                    my $y1 = int(
                        $y + cos( $sa + ( $ra * $i / $parts ) ) * $secondary );
                    my $x2 =
                      int( $x +
                          sin( $sa + ( $ra * ( $i + 1 ) / $parts ) ) *
                          $radius );
            ...
            elsif ( $d{'RECORD'} eq '34' )    #Designator
            {
                ...
                my ( $x, $y, $orient, $dir ) =
                  get_f_position( %d, $f, $partorientation{$globalp},
                    $relx, $rely, $sheety );

                my $desig = "IC";
                $desig = $1 if ( $d{'TEXT'} =~ m/^([A-Z]*)/ );
                my $ref = uniquify( $d{'TEXT'} );
                $ref =~ s/"/'/g;

                push @{ $parts{$globalp} },
                  "F 0 \"$ref\" $orient $x $y 60  0000 $dir\n";    # L BNN\n";

                $x                         = ( $d{'LOCATION.X'} * $f ) - $relx;
                $y                         = ( $d{'LOCATION.Y'} * $f ) - $rely;
                $globalreference{$globalp} = $ref;

#$refdesLabels{$LIBREFERENCE}="\"$desig\" $x $y 60 H V L BNN"; # $desig 70 H V L BNN
                $refdesLabels{$LIBREFERENCE} =
"<text x=\"$x\" y=\"$y\" size=\"1.27\" layer=\"95\" align=\"center\">&gt;NAME</text>\n";
                $refdes{$LIBREFERENCE} = $desig;
            }
                ...
                if ( !( defined( $d{'ISHIDDEN'} ) && $d{'ISHIDDEN'} eq 'T' ) ) {
                    if ( ( $d{'NAME'} || "" ) eq "Comment" ) {
                        ...
                        if ( defined( $d{'TEXT'} ) ) {
                            ...
                            if (
                                substr( $value, 0, 1 ) eq
                                '=' )    # It's an xref - look it up
                            {
                                my $paramname = substr( $value, 1 );
                                $value =
                                  $partparams{ lc($paramname) } || $value;
                            }
                            $value =~ s/"/'/g;
                            push @{ $parts{$globalp} },
                              "F 1 \"$value\" $orient $x $y 60  0000 $dir\n"
                              ;          #L BNN
                            push @{ $parts{$globalp} },
                              "F 2 \"\" H $x $y 60  0000 C CNN\n";
                            push @{ $parts{$globalp} },
                        ...
                        $nameLabels{$LIBREFERENCE} =
                          "\"$LIBREFERENCE\" $x $y 60 $orient V L BNN";
                        $nameLabels{$LIBREFERENCE} =
"<text x=\"$x\" y=\"$y\" size=\"1.27\" layer=\"96\" align=\"center\">&gt;VALUE</text>\n";
...
#print "globalp: $globalp OWNERINDEX: $d{OWNERINDEX}\n" if($d{'NAME'} eq "MPN"); I am not sure, whether the association through globalp is correct here
                            ...
                            $NAME =~ s/"/'/g;
                            push @{ $parts{$globalp} },
"F $counter \"$TEXT\" V 1400 2000 60  0001 C CNN \"$NAME\"\n";
...
#print "Error: Parameter $d{'NAME'}=$d{'TEXT'} without position!\n" if(defined($d{'TEXT'}) && $d{'TEXT'} ne "*");
            ...
            elsif ( $d{'RECORD'} eq '30' )    # Image
            {
                ...
                if ( ( -f $bmp ) && ( !-f $png ) ) {
                    system "\"$imagemagick"
                      . "convert\" -colorspace RGB \"$bmp\" \"$png\"";
                }
            ...
            elsif ( $d{'RECORD'} eq '31' )    #Sheet
            {
                ...
                foreach ( 1 .. $nfonts ) {
                    ...
                    my $fontsize = $d{ 'SIZE' . $_ };
                    $fontsize{$_} = $fontsize;
                    my $rotation = $d{ 'ROTATION' . $_ } || "0";
                    $fontrotation{$_} = $rotation;
                    my $bold = $d{ 'BOLD' . $_ } || "";
                    $fontbold{$_} = $bold;
            ...
            elsif ( $d{'RECORD'} eq '18' )    # Port
            {
                ...
                my $orientation =
                  ( $d{'ALIGNMENT'} || 0 ) > 2
                  ? 0
                  : 2;    # Altium seems to ignore this for harnesses?
                my $shape = $iotypes{ $d{'IOTYPE'} || 0 }
                  ;       # Altium never seems to write out IOTYPE=0 for BiDi's
                ...
                my $labeltype = "GLabel";
                my $size      = $fontsize{ $d{'FONTID'} } * 6;
                $name .= "_HARN", $labeltype = "HLabel"
                  if ( defined( $d{'HARNESSTYPE'} ) )
                  ;    # Annotated bodge for missing harness feature
            ...
            elsif ( $d{'RECORD'} eq '16' )    # sheet entry
            {
                ...
                my $y = $sheety - $rely;
                my $distance =
                  ( ( $d{'DISTANCEFROMTOP'} || 0 ) * 10 +
                      ( $d{'DISTANCEFROMTOP_FRAC1'} || 0 ) / 100000.0 ) *
                  $f;
                my $side = $d{'SIDE'} || 0;
                my $shape = $iotypes{ $d{'IOTYPE'} || 0 };
                my $name = $d{'NAME'};
                ...
                $dat .=
                  "Text HLabel $x $y ${orient} $size ${shape} ~\n${name}\n";
            }
            ...
            elsif ( $d{'RECORD'} eq '26' )    # BUS POLYLINE
            {
                ...
                print
                  "Unhandled Record type without: $d{RECORD}  (#$d{LINENO})\n";
            }

            print OUT $dat
              unless ( defined( $d{'ISHIDDEN'} ) && ( $d{'ISHIDDEN'} eq 'T' ) );
        }
    ...
    foreach my $part ( sort keys %parts ) {
        ...
        print OUT "L $partcomp{$part} "
          . ( $globalreference{$part} || "IC$ICcount" )
          . "\n";    # IC$ICcount\n";
    ...
    foreach my $component ( sort keys %componentdraw ) {
        my $comp = "<symbol name=\"$component\">\n";
        $comp .= $refdesLabels{$component};
        $comp .= $nameLabels{$component}||"";
        $comp .= $customfields{$component} || "";
    ...
    print LIB "</symbols>\n";
    print LIB "<devicesets>";
    foreach my $component ( sort keys %componentdraw ) {
        my $comp =
          "<device name=\"$component\" prefix=\"$refdes{$component}\">\n";
        $comp .= "<gates>\n";
        $comp .= "<gate name=\"G\$1\" symbol=\"$component\" x=\"0\" y=\"0\"/>";
        $comp .= "</gates>\n";
        $comp .= "</device>\n";
        print LIB $comp;
    }
...
foreach my $lib ( sort keys %rootlibraries ) {
    ...
    print LIB "</library></drawing></eagle>\n";
    close LIB;
}
