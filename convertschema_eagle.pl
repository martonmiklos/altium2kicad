...
;
          ...
          $refdeslabel{$device}="\"#PWR\" 0 140 20 H I L BNN";
...
;
	  ...
	  elsif($d{'RECORD'} eq '34') #Designator
	  {
		...
		#$refdeslabel{$LIBREFERENCE}="\"$desig\" $x $y 60 H V L BNN"; # $desig 70 H V L BNN
		$refdeslabel{$LIBREFERENCE}="<text x=\"$x\" y=\"$y\" size=\"1.27\" layer=\"95\" align=\"center\">&gt;NAME</text>\n";
  ...
  foreach my $component (sort keys %componentdraw)
  {
    my $comp="<device name=\"$component\" prefix=\"$refdes{$component}\">\n";
    $comp.="<gates>\n";
